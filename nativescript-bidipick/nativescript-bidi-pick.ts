...
export class NativescriptBidipick extends Common {
  ...
  public initCalendar(): any {
    ...
    calendarView.init(lastYear.getTime(), nextYear.getTime(), new SimpleDateFormat(options.simpleDateFormat, local), local, options.supportsRtl).inMode(CalendarPickerView.SelectionMode[options.selectionMode]).withHighlightedDates(arrayList).withSelectedDate(today);
...
export function create(_options?: Options) {
  options = _options;
  return new NativescriptBidipick();
  //  console.log(11)
...
registerElement('NativescriptBidipick', () => require('./').NativescriptBidipick);
